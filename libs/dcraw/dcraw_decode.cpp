 ...
 *
 * Original Raw Photo Decoder source code come from dcraw.c
 * Revision: 1.309 - Date: 2005/12/18 21:02:19
...
DcrawDecode::DcrawDecode()
{
    clip_color=1;
    bright=1, red_scale=1, blue_scale=1, sigma_d=0, sigma_r=0;
    four_color_rgb=0, document_mode=0;
    verbose=0, use_auto_wb=0, use_camera_wb=0, use_camera_rgb=0;
    use_secondary=0;
}
...
/***************************************************************
                Private methods from dcraw::dcraw.c
 **************************************************************/

void DcrawDecode::merror (void *ptr, char *where)
{
  if (ptr) return;
...
ushort DcrawDecode::sget2 (uchar *s)
{
...
double DcrawDecode::getrat()
{
  double num = get4();
  return num / get4();
}

float DcrawDecode::int_to_float (int i)
{
...
void DcrawDecode::read_shorts (ushort *pixel, int count)
{
...
void DcrawDecode::canon_600_fixed_wb (int temp)
{
...
int DcrawDecode::canon_600_color (int ratio[2], int mar)
{
...
void DcrawDecode::canon_600_auto_wb()
{
...
void DcrawDecode::canon_600_coeff()
{
...
void DcrawDecode::canon_600_load_raw()
{
  uchar  data[1120], *dp;
...
void DcrawDecode::canon_a5_load_raw()
{
  uchar  data[1940], *dp;
 ...
 */
unsigned DcrawDecode::getbits (int nbits)
{
...
/*
   ...
   then the code is
    ...
    1111111     0xff
 */
unsigned char* DcrawDecode::make_decoder (const unsigned char *source, int level)
{
  struct decode *cur;
  static int leaf;
  int i, next;
...
void DcrawDecode::crw_init_tables (unsigned table)
{
 ...
 */
int DcrawDecode::canon_has_lowbits()
{
  uchar test[0x4000];
  int ret=1, i;
...
void DcrawDecode::canon_compressed_load_raw()
{
  ushort *pixel, *prow;
...
int DcrawDecode::ljpeg_start (struct jhead *jh)
{
  int i, tag, len;
  uchar data[256], *dp;
...
int DcrawDecode::ljpeg_diff (struct decode *dindex)
{
  int len, diff;
...
void DcrawDecode::ljpeg_row (int jrow, struct jhead *jh)
{
  int col, c, diff;
  ushort *outp=jh->row;
...
void DcrawDecode::lossless_jpeg_load_raw()
{
...
void DcrawDecode::adobe_copy_pixel (int row, int col, ushort **rp)
{
  unsigned r, c;
...
void DcrawDecode::adobe_dng_load_raw_lj()
{
...
void DcrawDecode::adobe_dng_load_raw_nc()
{
  ushort *pixel, *rp;
  int row, col;
...
void DcrawDecode::nikon_compressed_load_raw()
{
...
void DcrawDecode::nikon_load_raw()
{
  int irow, row, col, i;
 ...
 */
int DcrawDecode::nikon_is_compressed()
{
  uchar test[256];
  int i;
...
void DcrawDecode::nikon_e900_load_raw()
{
...
void DcrawDecode::nikon_e2100_load_raw()
{
  uchar   data[3456], *dp;
 ...
 */
void DcrawDecode::fuji_load_raw()
{
  ushort *pixel;
  int row, col, r, c;
...
void DcrawDecode::rollei_load_raw()
{
  uchar pixel[10];
...
void DcrawDecode::phase_one_load_raw()
{
  int row, col, a, b;
...
unsigned DcrawDecode::ph1_bits (int nbits)
{
...
void DcrawDecode::phase_one_load_raw_c()
{
  ...
  ncols = (raw_width + 7) & -8;
  pixel = (ushort *)calloc (ncols, sizeof *pixel);
...
void DcrawDecode::leaf_load_raw()
{
  ushort *pixel;
  int r, c, row, col;
...
/* Here raw_width is in bytes, not pixels. */
void DcrawDecode::packed_12_load_raw()
{
  int row, col;
...
void DcrawDecode::unpacked_load_raw()
{
  ushort *pixel;
  int row, col;
...
void DcrawDecode::olympus_e300_load_raw()
{
  uchar  *data,  *dp;
  ushort *pixel, *pix;
  int dwide, row, col;
...
void DcrawDecode::olympus_cseries_load_raw()
{
  int irow, row, col;
...
void DcrawDecode::minolta_rd175_load_raw()
{
  uchar pixel[768];
...
void DcrawDecode::eight_bit_load_raw()
{
  uchar *pixel;
  int row, col;
...
void DcrawDecode::casio_qv5700_load_raw()
{
  uchar  data[3232],  *dp;
...
void DcrawDecode::nucore_load_raw()
{
  ushort *pixel;
  int irow, row, col;
...
const int * DcrawDecode::make_decoder_int (const int *source, int level)
{
  struct decode *cur;
...
int DcrawDecode::radc_token (int tree)
{
  int t;
...
void DcrawDecode::kodak_radc_load_raw()
{
...
// FIXME : This ststic method isn't thread safe

boolean DcrawDecode::fill_input_buffer (j_decompress_ptr cinfo)
{
  unsigned char jpeg_buffer[4096];
  size_t nbytes;

  DcrawDecode *pDCR = reinterpret_cast<DcrawDecode *>(cinfo->client_data);
  nbytes = fread ( jpeg_buffer, 1, 4096, pDCR->ifp);
...
void DcrawDecode::kodak_jpeg_load_raw()
{
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    pixel = reinterpret_cast<JSAMPLE (*)[3]>(buf[0]);
...
void DcrawDecode::kodak_dc120_load_raw()
{
...
void DcrawDecode::kodak_easy_load_raw()
{
  uchar *pixel;
...
void DcrawDecode::kodak_compressed_load_raw()
{
  uchar c, blen[256];
  ushort raw[6];
...
void DcrawDecode::kodak_yuv_load_raw()
{
  uchar c, blen[384];
...
void DcrawDecode::sony_decrypt (unsigned *data, int len, int start, int key)
{
...
void DcrawDecode::sony_load_raw()
{
  uchar head[40];
  ushort *pixel;
  ...
  fread (head, 1, 40, ifp);
  sony_decrypt (reinterpret_cast<unsigned int *>(head), 10, 1, key);
  for (i=26; i-- > 22; )
    key = key << 8 | head[i];
  fseek (ifp, data_offset, SEEK_SET);
  pixel = (ushort *)calloc (raw_width, sizeof *pixel);
  ...
  for (row=0; row < height; row++) {
    ...
    sony_decrypt (reinterpret_cast<unsigned int *>(pixel), raw_width/2, !row, key);
...
void DcrawDecode::smal_decode_segment (unsigned seg[2][2], int holes)
{
  uchar hist[3][13] = {
...
void DcrawDecode::smal_v6_load_raw()
{
  unsigned seg[2][2];
...
void DcrawDecode::fill_holes (int holes)
{
  int row, col, val[4];
...
void DcrawDecode::smal_v9_load_raw()
{
...
void DcrawDecode::foveon_decoder (unsigned huff[1024], unsigned code)
{
  struct decode *cur;
  int i, len;
...
void DcrawDecode::foveon_load_camf()
{
  unsigned key, i, val;
...
void DcrawDecode::foveon_load_raw()
{
...
char * DcrawDecode::foveon_camf_param (char *block, char *param)
{
  unsigned idx, num;
  char *pos, *cp, *dp;
...
void * DcrawDecode::foveon_camf_matrix (int dim[3], char *name)
{
  ...
  for (idx=0; idx < meta_length; idx += sget4(pos+8)) {
    ...
    mat = (unsigned int*)malloc (size * 4);
...
int DcrawDecode::foveon_fixed (void *ptr, int size, char *name)
{
  void *dp;
  int dim[3];
...
float DcrawDecode::foveon_avg (short *pix, int range[2], float cfilt)
{
  int i;
...
short * DcrawDecode::foveon_make_curve (double max, double mul, double filt)
{
  short *curve;
  int i, size;
  double x;
...
void DcrawDecode::foveon_make_curves(short **curvep, float dq[3], float div[3], float filt)
{
  double mul[3], max=0;
  int c;
...
int DcrawDecode::foveon_apply_curve (short *curve, int i)
{
...
void DcrawDecode::foveon_interpolate()
{
  ...
  foveon_fixed (color_dq, 3,
    foveon_camf_param ("IncludeBlocks", "ColorDQ") ?
        (char*)"ColorDQ" : (char*)"ColorDQCamRGB");
  ...
  if (!foveon_camf_param ("IncludeBlocks", "DarkDrift")
    ...
    for (i=0; i < 2; i++) {
      foveon_fixed (dstb, 4, i ? (char*)"DarkShieldBottom" : (char*)"DarkShieldTop");
  ...
  sgain = reinterpret_cast<float (*)[3]>(foveon_camf_matrix (dim, "SpatialGain"));
  if (!sgain) return;
  sgrow = reinterpret_cast<float (*)[3]>(calloc (dim[1], sizeof *sgrow));
  sgx = (width + dim[1]-2) / (dim[1]-1);

  black = reinterpret_cast<float (*)[3]>(calloc (height, sizeof *black));
  for (row=0; row < height; row++) {
    for (i=0; i < 6; i++)
  ...
  if (badpix = reinterpret_cast<unsigned int *>(foveon_camf_matrix (dim, "BadPixels"))) {
    for (i=0; i < dim[0]; i++) {
  ...
  smrow[6] = reinterpret_cast<int (*)[3]>(calloc (width*5, sizeof **smrow));
 ...
 */
  ...
  shrink = reinterpret_cast<short (*)[3]>(calloc ((width/4) * (height/4), sizeof *shrink));
...
void DcrawDecode::pseudoinverse (const double (*in)[3], double (*out)[3], int size)
{
  double work[3][6], num;
  int i, j, k;
...
void DcrawDecode::cam_xyz_coeff (double cam_xyz[4][3])
{
...
void DcrawDecode::border_interpolate (int border)
{
...
void DcrawDecode::lin_interpolate()
{
 ...
 */
void DcrawDecode::vng_interpolate()
{
  ...
  brow[4] = reinterpret_cast<ushort (*)[4]>(calloc (width*3, sizeof **brow));
...
void DcrawDecode::cam_to_cielab (ushort cam[4], float lab[3])
{
  int c, i, j, k;
  float r, xyz[3];
...
void DcrawDecode::ahd_interpolate()
{
 ...
 */
void DcrawDecode::bilateral_filter()
{
...
void DcrawDecode::parse_makernote()
{
 ...
 */
void DcrawDecode::get_timestamp (int reversed)
{
  struct tm t;
  char str[20];
  int i;
...
void DcrawDecode::parse_exif (int base)
{
...
int DcrawDecode::parse_tiff_ifd (int base, int level)
{
...
void DcrawDecode::parse_tiff (int base)
{
...
void DcrawDecode::parse_minolta()
{
 ...
 */
void DcrawDecode::parse_external_jpeg()
{
 ...
 */
void DcrawDecode::ciff_block_1030()
{
 ...
 */
void DcrawDecode::parse_ciff (int offset, int length)
{
...
void DcrawDecode::parse_mos (int offset)
{
  char data[40];
...
void DcrawDecode::parse_phase_one (int base)
{
...
void DcrawDecode::parse_fuji (int offset)
{
...
int DcrawDecode::parse_jpeg (int offset)
{
  int len, save, hlen;
...
void DcrawDecode::parse_smal (int offset, int fsize)
{
  int ver;
...
char * DcrawDecode::foveon_gets (int offset, char *str, int len)
{
  int i;
  fseek (ifp, offset, SEEK_SET);
  for (i=0; i < len-1; i++)
...
void DcrawDecode::simple_coeff (int index)
{
...
short DcrawDecode::guess_byte_order (int words)
{
  uchar test[4][2];
  int t=2, msb;
 ...
 */
int DcrawDecode::identify (int no_decode)
{
  char head[32], *cp;
...
void DcrawDecode::apply_profile (char *pfname)
{
  char *prof;
 ...
 */
void DcrawDecode::convert_to_rgb()
{
 ...
 */
void DcrawDecode::write_ppm (FILE *ofp)
{
 ...
 */
void DcrawDecode::write_psd (FILE *ofp)
{
  char head[] = {
    '8','B','P','S',        /* signature */
 ...
 */
void DcrawDecode::write_ppm16 (FILE *ofp)
{
  int row, col, c;
  ushort (*ppm)[3];
