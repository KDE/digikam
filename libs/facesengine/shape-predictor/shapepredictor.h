...
//        }
    ...
    // a tree is just a std::vector<redeye::splitfeature>.  We use this function to navigate the
    // tree nodes
    ...
    struct regressiontree
    {
        ...
        inline const std::vector<float>& operator()(
...
// ------------------------------------------------------------------------------------
    ...
    inline pointtransformaffine normalizing_tform (
        const cv::Rect& rect
    )
    /*!
        ensures
    ...
    !*/
    {
        //std::vector<vector<float,2> > from_points, to_points;
        std::vector<std::vector<float> > from_points, to_points;
        std::vector<float> tlcorner(2); tlcorner[0] = rect.x;
        tlcorner[1] = rect.y;
        std::vector<float> trcorner(2); tlcorner[0] = rect.x + rect.width;
        trcorner[1] = rect.y;
        std::vector<float> brcorner(2); tlcorner[0] = rect.x + rect.width;
        brcorner[1] = rect.y + rect.height;

        std::vector<float> pt1(2); pt1[0] = 0; pt1[1] = 0;
        std::vector<float> pt2(2); pt2[0] = 1; pt2[1] = 0;
        std::vector<float> pt3(2); pt3[0] = 1; pt3[1] = 1;
        from_points.push_back(tlcorner); to_points.push_back(pt1);
        from_points.push_back(trcorner); to_points.push_back(pt2);
        from_points.push_back(brcorner); to_points.push_back(pt3);
        return find_affine_transform(from_points, to_points);
    }

// ------------------------------------------------------------------------------------

    inline pointtransformaffine unnormalizing_tform (
        const cv::Rect& rect
    )
    /*!
        ensures
    ...
    !*/
    {
        ...
        std::vector<float> tlcorner(2); tlcorner[0] = rect.x;
        tlcorner[1] = rect.y;
        std::vector<float> trcorner(2); trcorner[0] = rect.x + rect.width;
        trcorner[1] = rect.y;
        std::vector<float> brcorner(2); brcorner[0] = rect.x + rect.width;
        brcorner[1] = rect.y + rect.height;
        ...
        //std::vector<vector<float,2> > from_points, to_points;
        to_points.push_back(tlcorner); from_points.push_back(pt1);
        to_points.push_back(trcorner); from_points.push_back(pt2);
        to_points.push_back(brcorner); from_points.push_back(pt3);
        return find_affine_transform(from_points, to_points);
    }
...
//        const std::vector<dlib::vector<float,2> >& reference_pixel_deltas,
    ...
    !*/
    {
        ...
        const cv::Rect area = cv::Rect(0,0,img_.size().width,
                                       img_.size().height);
