...
void TrainingDB::updateLBPHFaceModel(LBPHFaceModel& model)
{
    ...
    for (int i = 0 ; i < metadataList.size() ; i++)
    {
        ...
        if (metadata.storageStatus == LBPHistogramMetadata::Created)
        {
            ...
            {
                qCWarning(DIGIKAM_GENERAL_LOG) << "Histogram data to commit in database are empty for Identity " << metadata.identity;
            }
            else
            {
                ...
                {
                    qCWarning(DIGIKAM_GENERAL_LOG) << "Cannot compress histogram data to commit in database for Identity " << metadata.identity;
                }
                else
                {
                    ...
                    qCDebug(DIGIKAM_GENERAL_LOG) << "Commit compressed histogram " << metadata.databaseId << " for identity " << metadata.identity << " with size " << compressed.size();
                }
            }
        }
    }
...
LBPHFaceModel TrainingDB::lbphFaceModel() const
{
    ...
    for (QList<QVariant>::const_iterator it = values.constBegin(); it != values.constEnd();)
    {
        ...
        {
            qCDebug(DIGIKAM_GENERAL_LOG) << "Unsupported LBPH storage version" << version;
            it += 4;
            continue;
        }
        ...
        while (query.next())
        {
            ...
            if (!cData.isEmpty())
            {
                ...
                if (data.data.isEmpty())
                {
                    qCWarning(DIGIKAM_GENERAL_LOG) << "Cannot uncompress histogram data to checkout from database for Identity " << metadata.identity;
                }
                else
                {
                    qCDebug(DIGIKAM_GENERAL_LOG) << "Checkout compressed histogram " << metadata.databaseId << " for identity " << metadata.identity << " with size " << cData.size();
            ...
            {
                qCWarning(DIGIKAM_GENERAL_LOG) << "Histogram data to checkout from database are empty for Identity " << metadata.identity;
            }
        }
