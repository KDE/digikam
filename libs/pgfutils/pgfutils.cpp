...
bool readPGFImageData(const QByteArray& data, QImage& img, bool verbose)
{
    try
    {
        ...
        if (verbose) qDebug() << "PGFUtils: image data stream size is : " << stream.GetSize();
        ...
        if (verbose) qDebug() << "PGFUtils: PGF image is open";
        ...
        if (verbose) qDebug() << "PGFUtils: PGF image is read";
        ...
        if (verbose) qDebug() << "PGFUtils: PGF image is decoded";
    }
    catch (IOException& e)
    {
        int err = e.error;
...
bool writePGFImageFile(const QImage& image, const QString& filePath, int quality, bool verbose)
{
    ...
    {
        if (verbose) qDebug() << "PGFUtils: file size written : " << nWrittenBytes;
    }
...
bool writePGFImageDataToStream(const QImage& image, CPGFStream& stream, int quality, UINT32& nWrittenBytes, bool verbose)
{
    try
    {
        ...
        if (image.format() != QImage::Format_ARGB32)
        {
            ...
            if (verbose) qDebug() << "PGFUtils: RGB => ARGB";
        }
        else
        {
            img = image;
