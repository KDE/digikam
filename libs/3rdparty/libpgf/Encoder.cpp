...
// Throws IOException
void CEncoder::EncodeInterleaved(CWaveletTransform* wtChannel, int level, int quant) THROW_ {
	ASSERT(wtChannel);
	...
	// quantize subbands
	hlBand->Quantize(quant);
	lhBand->Quantize(quant);
		...
		for (j=0; j < hlW.quot; j++) {
			...
			for (y=0; y < InterBlockSize; y++) {
				for (x=0; x < InterBlockSize; x++) {
					WriteValue(hlBand, hlPos);
					WriteValue(lhBand, lhPos);
					hlPos++;
					lhPos++;
				}
				hlPos += hlws;
...
void CEncoder::WriteTileValue(CSubband* band, int bandPos) THROW_ {
	...
	// store absolut value of wavelet coefficient in buffer m_value
	DataT v = band->GetData(bandPos);
	UINT32 absv = abs(v);
	m_value[m_valuePos] = absv;
	if (absv > m_maxAbsValue) m_maxAbsValue = absv;

	// store sign of wavelet coefficient in m_sign
	if (v < 0) {
		SetBit(m_sign, m_valuePos);
	} else {
		ClearBit(m_sign, m_valuePos);
	}
...
///////////////////////////////////////////////////////
// Encode Buffer and write data into file
// Throws IOException
void CEncoder::EncodeTileBuffer() THROW_ {
	ASSERT(m_valuePos >= 0 && m_valuePos <= BufferSize);

	UINT32 codePos = 0;					// bit position in m_codeBuffer
	UINT32 sigPos = 0, refPos = 0;		// bit positions
	UINT32 codeLen;						// in bits
	UINT32 sigBits[BufferSize] = { 0 };	// buffer for significant bits
	UINT32 refBits[BufferSize];			// buffer for refinement bits
	UINT8 nPlanes = NumberOfBitPlanes();
	...
	if (m_valuePos == BufferSize) {
		...
		h.eofTile = 1;
		h.blockSize = m_valuePos;
	}
	h.nPlanes = nPlanes;

	if (m_valuePos > 0) {
		// bit plane decomposition: split buffer into separate bit planes
		DecomposeTileBuffer(nPlanes, m_valuePos, sigBits, sigPos, refBits, refPos);

		// RLE encoding of sigBits and m_sign: encoded data is stored in m_codeBuffer at codePos
		codeLen = RLEncode(sigBits, sigPos, codePos);
		
		if (AlignWordPos(codeLen) < AlignWordPos(sigPos) + AlignWordPos(m_valuePos)) {
			// RL encoding was helpful, because encoded data is shorter than significant bits together with m_sign
			h.rle = 1;
			h.sigLen = NumberOfWords(codeLen);
			h.codeLen = h.sigLen + NumberOfWords(refPos);
			// store refinement bits
			memcpy(&m_codeBuffer[h.sigLen], refBits, DWWIDTHBITS(refPos));
		} else {
			// we don't use RL encoding
			h.rle = 0;
			h.sigLen = NumberOfWords(sigPos);
			h.codeLen = h.sigLen + NumberOfWords(m_valuePos) + NumberOfWords(refPos);
			// overwrite significant bits
			codePos = 0;
			memcpy(m_codeBuffer, sigBits, DWWIDTHBITS(sigPos));
			// store m_sign
			memcpy(&m_codeBuffer[h.sigLen], m_sign, DWWIDTHBITS(m_valuePos));
			// store refinement bits
			memcpy(&m_codeBuffer[h.sigLen + NumberOfWords(m_valuePos)], refBits, DWWIDTHBITS(refPos));
		}
	} else {
		h.rle = 0;
		h.sigLen = 0;
	...
	h.blockSize = __VAL(h.blockSize);
	h.codeLen = __VAL(codeLen);
	h.sigLen = __VAL(h.sigLen);
#endif
	...
	count = codeLen*WordBytes;
	m_stream->Write(&count, m_codeBuffer);

	// store length of level
	if (m_levelLength && m_isLevelEncoded) {
		m_isLevelEncoded = false;
		UINT64 filePos = m_stream->GetPos();
		m_levelLength[m_currLevelIndex++] = UINT32(filePos - m_currPosition);
		m_currPosition = filePos;
	}

	// reset buffer position and maxAbsValue
	m_valuePos = 0;
	m_maxAbsValue = 0;
}

//////////////////////////////////////////////////////////
// Split buffer m_value into significant and refinement bitset
// input:  nPlanes, bufferSize, m_value
// output: m_sigFlagVector, sigBits, refBits
void CEncoder::DecomposeTileBuffer(UINT8 nPlanes, UINT32 bufferSize, UINT32* sigBits, UINT32& sigPos, UINT32* refBits, UINT32& refPos) {
	UINT32 valuePos, valueEnd, sigRunLen;

	// clean m_sigFlagVector
	for (int k=0; k < BufferLen; k++) {
		m_sigFlagVector[k] = 0;
	}

	// go through all bit planes
	if (nPlanes == 0) nPlanes = MaxBitPlanes + 1;
	UINT32 planeMask = 1 << (nPlanes - 1);
	...
	for (int plane = nPlanes - 1; plane >= 0; plane--) {
		...
		while (valuePos < bufferSize) {
			...
			// search 0's in m_value[plane][valuePos..valueEnd)
			// these 0's are significant bits
			while (valuePos < valueEnd) {
				// search 0's
				while (valuePos < valueEnd && !GetBitAtPos(valuePos, planeMask)) {
					valuePos++;
