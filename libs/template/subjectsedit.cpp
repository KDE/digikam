...
#include "subjectsedit.h"
#include "subjectsedit.moc"
...
SubjectsEdit::SubjectsEdit(QWidget* parent)
            : QWidget(parent), d(new SubjectsEditPriv)
{
    ...
    d->btnGroup->addButton(d->stdBtn,    SubjectsEditPriv::STANDARD);
    d->btnGroup->addButton(d->customBtn, SubjectsEditPriv::CUSTOM);
    d->btnGroup->setExclusive(true);
    d->stdBtn->setChecked(true);

    for (SubjectsEditPriv::SubjectCodesMap::Iterator it = d->subMap.begin();
         it != d->subMap.end(); ++it)
...
SubjectsEdit::~SubjectsEdit()
{
    delete d;
}

void SubjectsEdit::slotSubjectsToggled(bool b)
{
...
void SubjectsEdit::slotEditOptionChanged(int b)
{
    if (b == SubjectsEditPriv::CUSTOM)
    {
...
void SubjectsEdit::slotRefChanged()
{
...
QString SubjectsEdit::buildSubject() const
{
...
void SubjectsEdit::slotDelSubject()
{
...
void SubjectsEdit::slotRepSubject()
{
...
void SubjectsEdit::slotSubjectSelectionChanged()
{
    if (!d->subjectsBox->selectedItems().isEmpty())
    {
...
void SubjectsEdit::slotAddSubject()
{
    QString newSubject = buildSubject();
    if (newSubject.isEmpty()) return;
...
bool SubjectsEdit::loadSubjectCodesFromXML(const KUrl& url)
{
         ...
         it != d->subMap.end(); ++it)
    {
        ...
        if (it.key().endsWith("00000"))
        {
            ...
            for (SubjectsEditPriv::SubjectCodesMap::Iterator it2 = d->subMap.begin();
...
void SubjectsEdit::setSubjectsEditList(const QStringList& list)
{
    d->subjectsList = list;
...
QStringList SubjectsEdit::subjectsList() const
{
    QStringList newSubjectsEdit;

    for (int i = 0 ; i < d->subjectsBox->count(); i++)
    {
        QListWidgetItem *item = d->subjectsBox->item(i);
        newSubjectsEdit.append(item->text());
    }

    return newSubjectsEdit;
}
