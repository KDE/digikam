...
bool JP2KLoader::load(const QString& filePath, DImgLoaderObserver* const observer)
{
    ...
    if (init != 0)
    {
        qCWarning(LOG_DIMG_JP2K) << "Unable to init JPEG2000 decoder";
        loadingFailed();
        return false;
    }
    ...
    if (jp2_stream == 0)
    {
        qCWarning(LOG_DIMG_JP2K) << "Unable to open JPEG2000 stream";
        loadingFailed();
        return false;
    }
    ...
    if (jp2_image == 0)
    {
        ...
        qCWarning(LOG_DIMG_JP2K) << "Unable to decode JPEG2000 image";
        loadingFailed();
        return false;
    }
    ...
    switch (jas_clrspc_fam(jas_image_clrspc(jp2_image)))
    {
        case JAS_CLRSPC_FAM_RGB:
        {
            ...
            if ((components[0] < 0) || (components[1] < 0) || (components[2] < 0))
            {
                ...
                qCWarning(LOG_DIMG_JP2K) << "Error parsing JPEG2000 image : Missing Image Channel";
                loadingFailed();
                return false;
            }
        ...
        case JAS_CLRSPC_FAM_GRAY:
        {
            ...
            {
                jas_image_destroy(jp2_image);
                qCWarning(LOG_DIMG_JP2K) << "Error parsing JP2000 image : Missing Image Channel";
                loadingFailed();
                return false;
            }
        ...
        {
            jas_image_destroy(jp2_image);
            qCWarning(LOG_DIMG_JP2K) << "Error parsing JP2000 image : Colorspace Model Is Not Supported";
            loadingFailed();
            return false;
        }
    }
    ...
    for (i = 0; i < (long)number_components; ++i)
    {
        ...
        {
            jas_image_destroy(jp2_image);
            qCWarning(LOG_DIMG_JP2K) << "Error parsing JPEG2000 image : Irregular Channel Geometry Not Supported";
            loadingFailed();
            return false;
        }
    ...
    for (i = 0; i < (long)number_components; ++i)
    {
        ...
        {
            jas_image_destroy(jp2_image);
            qCWarning(LOG_DIMG_JP2K) << "Error decoding JPEG2000 image data : Memory Allocation Failed";
            loadingFailed();
            return false;
        }
    }
    ...
    if (m_loadFlags & LoadImageData)
    {
        ...
        for (y = 0 ; y < (long)imageHeight() ; ++y)
        {
            for (i = 0 ; i < (long)number_components; ++i)
            {
                ...
                if (ret != 0)
                {
                    qCWarning(LOG_DIMG_JP2K) << "Error decoding JPEG2000 image data";
                    jas_image_destroy(jp2_image);
...
bool JP2KLoader::save(const QString& filePath, DImgLoaderObserver* const observer)
{
    ...
    {
        jas_stream_close(jp2_stream);
        qCWarning(LOG_DIMG_JP2K) << "Unable to create JPEG2000 image";
        return false;
    }
    ...
    for (i = 0 ; i < (long)number_components ; ++i)
    {
        ...
        if (pixels[i] == 0)
        {
            ...
            jas_image_destroy(jp2_image);
            qCWarning(LOG_DIMG_JP2K) << "Error encoding JPEG2000 image data : Memory Allocation Failed";
            return false;
        }
    }
    ...
    for (y = 0 ; y < (long)imageHeight() ; ++y)
    {
        ...
        for (i = 0 ; i < (long)number_components ; ++i)
        {
            ...
            if (ret != 0)
            {
                qCWarning(LOG_DIMG_JP2K) << "Error encoding JPEG2000 image data";
    ...
    qCDebug(LOG_DIMG_JP2K) << "JPEG2000 quality: " << quality;
    qCDebug(LOG_DIMG_JP2K) << "JPEG2000 "          << rateBuffer;

    int ret = jp2_encode(jp2_image, jp2_stream, rateBuffer);

    if (ret != 0)
    {
        qCWarning(LOG_DIMG_JP2K) << "Unable to encode JPEG2000 image";
