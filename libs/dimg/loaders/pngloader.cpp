...
bool PNGLoader::load(const QString& filePath, DImgLoaderObserver *observer)
{
    ...
    {
        kDebug(50003) << "Not a PNG image file." << endl;
        fclose(f);
        return false;
    }
    rewind(f);
    ...
    if (!png_ptr)
    {
        kDebug(50003) << "Invalid PNG image file structure." << endl;
        fclose(f);
        return false;
    }
    ...
    if (!info_ptr)
    {
        kDebug(50003) << "Cannot reading PNG image file structure." << endl;
...
bool PNGLoader::save(const QString& filePath, DImgLoaderObserver *observer)
{
    ...
    if (!png_ptr)
    {
        kDebug(50003) << "Invalid target PNG image file structure." << endl;
        fclose(f);
        return false;
    }
    ...
    if (info_ptr == NULL)
    {
        kDebug(50003) << "Cannot create PNG image file structure." << endl;
                                ...
                                char *profile_data, png_uint_32 length)
{
    ...
    kDebug(50003) << "Writing Raw profile: type=" << profile_type << ", length=" << length << endl;
