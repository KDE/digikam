    ...
    if (m_loadFlags & LoadImageData)
    {
        ...
        if (bits_per_sample == 16)          // 16 bits image.
        {
            ...
            for (tstrip_t st=0; st < num_of_strips; st++)
            {
                ...
                if (bytesRead == -1)
                {
                    DDebug() << "Failed to read strip" << endl;
                    delete [] data;
                    TIFFClose(tif);
                    return false;
                }
        ...
        else       // Non 16 bits images ==> get it on BGRA 8 bits.
        {
            ...
            {
                DDebug() << "Failed to set up RGBA reading of image, filename "
            ...
            for (uint row = 0; row < h; row += rows_per_strip)
            {
                ...
                {
                    DDebug() << "Failed to read image data" << endl;
                    delete [] data;
                    delete [] strip;
                    TIFFClose(tif);
                    return false;
...
*/
    ...
    for (y = 0; y < h; y++)
    {
        ...
        {
            DDebug() << "Cannot write main image to target file." << endl;
            _TIFFfree(buf);
            TIFFClose(tif);
            return false;
        }
    ...
    if (!bufThumb)
    {
        DDebug() << "Cannot allocate memory buffer for thumbnail." << endl;
        TIFFClose(tif);
        return false;
    }
    ...
    for (y = 0 ; y < uint32(thumb.height()) ; y++)
    {
        ...
        {
            DDebug() << "Cannot write thumbnail to target file." << endl;
            _TIFFfree(bufThumb);
            TIFFClose(tif);
            return false;
        }
