...
ParserList MainParser::parsers() const
{
    return m_parsers;
}

void MainParser::registerParser(Parser* parser)
{
    if (!parser)
        return;
...
MainParser::TokenMap MainParser::tokenMap(const QString& parseString)
{
    TokenMap tokenMap;
...
QString MainParser::parse(const QString& parseString, ParseInformation& info)
{
    if (!Parser::stringIsValid(parseString))
    {
...
void MainParser::replaceMatchingTokens(QString& parseString, QStringList& tokens, TokenMap* map)
{
...
int MainParser::extractTokens(QString& parseString, QStringList& tokens)
{
...
void MainParser::addTokenMapItem(int index, int length, const QString& value, TokenMap* map)
{
    if (!map)
        return;
