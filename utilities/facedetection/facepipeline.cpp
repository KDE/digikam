...
Benchmarker::Benchmarker(FacePipeline::FacePipelinePriv* d)
    : totalImages(0),
      faces(0),
      totalPixels(0),
      facePixels(0),
...
void Benchmarker::process(FacePipelineExtendedPackage::Ptr package)
{
    if (package->databaseFaces.isEmpty())
    {
        ...
        QList<DatabaseFace> groundTruth = d->iface->databaseFaces(package->info.id());
        QList<DatabaseFace> testedFaces = d->iface->toDatabaseFaces(package->detectionImage, package->info.id(), package->faces);
