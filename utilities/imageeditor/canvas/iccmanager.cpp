            ...
            : d(new IccManagerPriv)
{
    ...
    {
        kError(50003) << "Cannot open workspace color profile" << d->workspaceProfile.filePath();
        return;
    }
    ...
    if (!d->embeddedProfile.isNull() && !d->embeddedProfile.open())
    {
        ...
        kWarning(50003) << "Encountered invalid embbeded color profile in file" << d->filePath;
    }
...
IccProfile IccManager::imageProfile(ICCSettingsContainer::Behavior behavior, IccProfile specifiedProfile)
{
    ...
    {
        kError(50003) << "Let the RAW loader do automatic color conversion";
        return IccProfile();
    }
...
void IccManager::transformForOutput(const IccProfile& prof)
{
    ...
    {
        // set appropriate outputColorSpace in RawLoadingSettings
        kError(50003) << "Do not use transformForOutput for uncalibrated data "
                         "but let the RAW loader do the conversion to sRGB";
    }
    else if (isMissingProfile())
    {
