...
//     qCDebug(GEOIFACE_LOG)<<"updateClusters starting...";
...
//         qCDebug(GEOIFACE_LOG)<<i<<tile1Point<<tile2Point<<(tile1Point-tile2Point);
//     }
...
//         qCDebug(GEOIFACE_LOG)<<QString::fromLatin1("pixel at: %1, %2 (%3): %4 markers").arg(tilePoint.x()).arg(tilePoint.y()).arg(linearIndex).arg(pixelCountGrid[linearIndex]);
    }
...
//                         lastTooCloseClusterIndex = i;
        ...
        qCDebug(GEOIFACE_LOG)<<QString::fromLatin1("created cluster %1: %2 tiles").arg(s->clusterList.size()).arg(cluster.tileIndicesList.count());
...
//     qCDebug(GEOIFACE_LOG)<<s->clusterList.size();
    qCDebug(GEOIFACE_LOG)<<QString::fromLatin1("level %1: %2 non empty tiles sorted into %3 clusters (%4 searched)").arg(markerLevel).arg(debugCountNonEmptyTiles).arg(s->clusterList.count()).arg(debugTilesSearched);
