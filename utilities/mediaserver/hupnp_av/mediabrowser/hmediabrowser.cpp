    ...
    HContentDirectoryAdapter*, const QString& data)
{
    ...
    while(!reader.atEnd() && reader.readNextStartElement())
    {
        ...
        QString objId = attrs.value(QLatin1String("objID")).toString();
        QString updateId = attrs.value(QLatin1String("updateID")).toString();
        QString stUpdated = attrs.value(QLatin1String("stUpdate")).toString();

        HCdsLastChangeInfo info;
        if (name == QLatin1String("objAdd"))
        {
            QString parentId = attrs.value(QLatin1String("objParentID")).toString();
            QString objClass = attrs.value(QLatin1String("objClass")).toString();

            info = HCdsLastChangeInfo(objId, HCdsLastChangeInfo::ObjectAdded);
            info.setParentId(parentId);
            info.setObjectClass(objClass);
        }
        else if (name == QLatin1String("objMod"))
        {
            info = HCdsLastChangeInfo(objId, HCdsLastChangeInfo::ObjectModified);
        }
        else if (name == QLatin1String("objDel"))
        {
            info = HCdsLastChangeInfo(objId, HCdsLastChangeInfo::ObjectDeleted);
        }
        else
        {
            HLOG_WARN(QString::fromUtf8("Encountered unknown XML element: [%1]").arg(name.toString()));
        }
...
bool HMediaBrowser::browse(const HBrowseParams& loadParams)
{
    if (!isReady())
    {
        ...
        h_ptr->m_lastErrorDescription = QLatin1String("Media Browse is not ready");
        return false;
    }
    else if (!loadParams.isValid())
    {
        h_ptr->m_lastErrorCode = UpnpInvalidArgs;
        h_ptr->m_lastErrorDescription = QLatin1String("Invalid arguments");
        return false;
    }

    HBrowseParams params = loadParams;
    if (!params.filter().contains(QLatin1String("res")))
    {
        params.setFilter(QSet<QString>(params.filter()) << QLatin1String("res"));
    }
...
{
    HBrowseParams loadParams(QLatin1String("0"), HBrowseParams::ObjectAndChildrenRecursively);
    return browse(loadParams);
}

bool HMediaBrowser::browseRoot()
{
    HBrowseParams loadParams(QLatin1String("0"), HBrowseParams::ObjectAndDirectChildren);
    return browse(loadParams);
}
