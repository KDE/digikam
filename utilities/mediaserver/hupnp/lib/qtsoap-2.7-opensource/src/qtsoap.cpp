...
{
    t = Other;
    errorStr = QLatin1String("Unknown error");
}
...
QString QtSoapArray::arraySizeString() const
{
    ...
    if (siz0 != -1) {
        ...
        if (order > 1) arraySize += QLatin1String(",") + QString::number(siz1);
        if (order > 2) arraySize += QLatin1String(",") + QString::number(siz2);
        if (order > 3) arraySize += QLatin1String(",") + QString::number(siz3);
        if (order > 4) arraySize += QLatin1String(",") + QString::number(siz4);
    }

    arraySize += QLatin1String("]");
    return arraySize;
}
...
QDomElement QtSoapArray::toDomElement(QDomDocument doc) const
{
    ...
    QDomElement a = n.uri() == QLatin1String("")
                    ? doc.createElement( n.name())
                    : doc.createElementNS(n.uri(), prefix + QLatin1String(":") + n.name());

    QString schemaprefix = QtSoapNamespaces::instance().prefixFor(QLatin1String(XML_SCHEMA_INSTANCE));
    a.setAttributeNS(QLatin1String(XML_SCHEMA_INSTANCE), schemaprefix + QLatin1String(":type"), QLatin1String("xsd:Array"));

    QString encprefix = QtSoapNamespaces::instance().prefixFor(QLatin1String(SOAPv11_ENCODING));
    a.setAttributeNS(QLatin1String(SOAPv11_ENCODING), encprefix + QLatin1String(":arrayType"), QLatin1String("xsd:") + arrayTypeString());
    ...
    for (QtSoapArrayIterator i(*const_cast<QtSoapArray *>(this)); !i.atEnd(); ++i) {
        ...
        QString position = QLatin1String("[") + QString::number(pos0);
        if (order > 1) position += QLatin1String(",") + QString::number(pos1);
        if (order > 2) position += QLatin1String(",") + QString::number(pos2);
        if (order > 3) position += QLatin1String(",") + QString::number(pos3);
        if (order > 4) position += QLatin1String(",") + QString::number(pos4);
        position += QLatin1String("]");

        QString envprefix = QtSoapNamespaces::instance().prefixFor(QLatin1String(SOAPv11_ENVELOPE));
        item.setAttributeNS(QLatin1String(SOAPv11_ENVELOPE), envprefix + QLatin1String(":position"), position);
        a.appendChild(item);
    }
...
bool QtSoapStruct::parse(QDomNode node)
{
    ...
    for (int i = 0; i < c; ++i) {
        ...
        if (n.isComment())
            continue;
        if (!n.isElement()){
            errorStr = QLatin1String("In the struct element ") + e.tagName();
            errorStr += QLatin1String(", the ") + QString::number(i) + QLatin1String("th child ");
            errorStr += QLatin1String("is not an element.");
            return false;
        }
        ...
        if (!type.ptr()) {
            errorStr = QLatin1String("In the struct element ") + e.tagName();
            errorStr += QLatin1String(", child #") + QString::number(i) + QLatin1String(", ");
            errorStr += n.toElement().tagName() + QLatin1String(", was not recognized as a SOAP type.");
            return false;
        }
...
{
    return at(QtSoapQName(key, QLatin1String("")));
}
...
QDomElement QtSoapSimpleType::toDomElement(QDomDocument doc) const
{
    ...
    QString schemaprefix = QtSoapNamespaces::instance().prefixFor(QLatin1String(XML_SCHEMA_INSTANCE));
    a.setAttributeNS(QLatin1String(XML_SCHEMA_INSTANCE), schemaprefix + QLatin1String(":type"), QLatin1String("xsd:") + typeName());
...
bool QtSoapSimpleType::parse(QDomNode node)
{
    ...
    QDomAttr typeattr = e.attributeNode(QLatin1String("type"));
    QString type = typeattr.isNull() ? QString(QLatin1String("string")) : localName(typeattr.value()).toLower();
    ...
    case UnsignedShort:
    case UnsignedByte:
        if (e.text() == QLatin1String("") || (e.text() != QLatin1String("") && (e.text()[0].isNumber() || e.text()[0] == QLatin1Char('-'))))
            v = QVariant(e.text().toInt());
        else {
            errorStr = QLatin1String("Type error at element \"") + e.tagName() + QLatin1String("\"");
            return false;
        }
    ...
    case Boolean: {
        ...
        if (val == QLatin1String("false"))
            v = QVariant(false);
        else if (val == QLatin1String("true"))
            v = QVariant(true);
    }
        break;
    default:
...
QtSoapMessage::QtSoapMessage()
    : type(OtherType), envelope(QtSoapQName(QLatin1String("Envelope"), QLatin1String(SOAPv11_ENVELOPE)))
{
    init();
}
...
{
    QtSoapNamespaces::instance().registerNamespace(QLatin1String("SOAP-ENV"), QLatin1String(SOAPv11_ENVELOPE));
    QtSoapNamespaces::instance().registerNamespace(QLatin1String("SOAP-ENC"), QLatin1String(SOAPv11_ENCODING));
    QtSoapNamespaces::instance().registerNamespace(QLatin1String("xsi"), QLatin1String(XML_SCHEMA_INSTANCE));
    QtSoapNamespaces::instance().registerNamespace(QLatin1String("xsd"), QLatin1String(XML_SCHEMA));
}
...
bool QtSoapMessage::setContent(const QByteArray &buffer)
{
    ...
    if (!doc.setContent(buffer, true, &errorMsg,
        ...
        setFaultString(QLatin1String("XML parse error"));
        addFaultDetail(new QtSoapSimpleType(QtSoapQName(QLatin1String("ParseError")), s));
        return false;
    }
...
bool QtSoapMessage::isValidSoapMessage(const QDomDocument &candidate)
{
    ...
    if (localName(tmpe.tagName()).toUpper() != QLatin1String("ENVELOPE")) {
        setFaultCode(VersionMismatch);
        setFaultString(QLatin1String("SOAP structure invalid"));
        addFaultDetail(new QtSoapSimpleType(QtSoapQName(QLatin1String("extra")), QLatin1String("root element \"") + tmpe.localName()
                                        + QLatin1String("\"/\"") + tmpe.tagName() + QLatin1String("\" is not envelope")));
        return false;
    }
    ...
    if (tmp.isNull() || !tmp.isElement()) {
        setFaultCode(VersionMismatch);
        setFaultString(QLatin1String("SOAP structure invalid"));
        addFaultDetail(new QtSoapSimpleType(QtSoapQName(QLatin1String("extra")), QLatin1String("mandatory body element missing")));
        return false;
    }
    ...
    if (localName(tmpe2.tagName()).toUpper() == QLatin1String("HEADER")) {
        foundHeader = true;
        tmp = tmp.nextSibling();
    }
    ...
    if (localName(tmpe3.tagName()).toUpper() != QLatin1String("BODY")) {
        setFaultCode(VersionMismatch);
        setFaultString(QLatin1String("SOAP structure invalid"));
        addFaultDetail(new QtSoapSimpleType(QtSoapQName(QLatin1String("extra")), QLatin1String("mandatory body element missing")));
        return false;
    ...
    // correct.
    if (tmpe.namespaceURI() != QLatin1String(SOAPv11_ENVELOPE)) {
        setFaultCode(VersionMismatch);
        setFaultString(QLatin1String("SOAP structure invalid"));
        addFaultDetail(new QtSoapSimpleType(QtSoapQName(QLatin1String("extra")), QLatin1String("Unsupported namespace for envelope element")));
        return false;
    }
...
QString QtSoapMessage::toXmlString(int indent) const
{
    ...
    QDomDocument doc = impl.createDocument(QString(), QString(QLatin1String("placeholder")),
                                           QDomDocumentType());
    ...
    env.setAttribute(QtSoapNamespaces::instance().prefixFor(QLatin1String(SOAPv11_ENVELOPE))
                     + QLatin1String(":") + QLatin1String("encodingStyle"),
                     QLatin1String(SOAPv11_ENCODING));

    env.setAttribute(QLatin1String("xmlns:") + QtSoapNamespaces::instance().prefixFor(QLatin1String(XML_SCHEMA)),
                     QLatin1String(XML_SCHEMA));
...
{
    QtSoapType &headerTmp = envelope[QtSoapQName(QLatin1String("Header"), QLatin1String(SOAPv11_ENVELOPE))];
    if (!headerTmp.isValid())
        envelope.insert(new QtSoapStruct(QtSoapQName(QLatin1String("Header"), QLatin1String(SOAPv11_ENVELOPE))));

    QtSoapStruct &header = (QtSoapStruct &)envelope[QtSoapQName(QLatin1String("Header"), QLatin1String(SOAPv11_ENVELOPE))];
    header.insert(item);
}
/*!
...
{
    return body()[QtSoapQName(QLatin1String("Fault"), QLatin1String(SOAPv11_ENVELOPE))][QtSoapQName(QLatin1String("Detail"))];
}
...
{
    return body()[QtSoapQName(QLatin1String("Fault"), QLatin1String(SOAPv11_ENVELOPE))][QtSoapQName(QLatin1String("Faultstring"))];
}
...
{
    QtSoapType &code = body()[QtSoapQName(QLatin1String("Fault"))][QtSoapQName(QLatin1String("Faultcode"))];
    ...
    int pos;
    if ((pos = fcodestr.indexOf(QLatin1Char('.'))) != -1)
        fcodestr.truncate(pos);

    if (localName(fcodestr.toLower()) == QLatin1String("versionmismatch"))
        return VersionMismatch;

    if (localName(fcodestr.toLower()) == QLatin1String("mustunderstand"))
        return MustUnderstand;

    if (localName(fcodestr.toLower()) == QLatin1String("client"))
        return Client;

    if (localName(fcodestr.toLower()) == QLatin1String("server"))
        return Server;
...
{
    const QtSoapQName bodyName(QLatin1String("Body"), QLatin1String(SOAPv11_ENVELOPE));
...
{
    const QtSoapQName headerName(QLatin1String("Header"), QLatin1String(SOAPv11_ENVELOPE));
...
void QtSoapMessage::setFaultCode(FaultCode code)
{
    ...
    if (!body()[QtSoapQName(QLatin1String("Fault"), QLatin1String(SOAPv11_ENVELOPE))].isValid())
        addBodyItem(new QtSoapStruct(QtSoapQName(QLatin1String("Fault"), QLatin1String(SOAPv11_ENVELOPE))));

    QString codeStr;
    switch (code) {
    case VersionMismatch:
        codeStr = QLatin1String("SOAP-ENV:VersionMismatch");
        break;
    case MustUnderstand:
        codeStr = QLatin1String("SOAP-ENV:MustUnderstand");
        break;
    case Client:
        codeStr = QLatin1String("SOAP-ENV:Client");
        break;
    case Server:
        codeStr = QLatin1String("SOAP-ENV:Server");
        break;
    case Other:
        codeStr = QLatin1String("Other");
        break;
    }

    QtSoapType &node = body()[QtSoapQName(QLatin1String("Fault"), QLatin1String(SOAPv11_ENVELOPE))];
    QtSoapStruct &fault = reinterpret_cast<QtSoapStruct &>(node);
    fault.insert(new QtSoapSimpleType(QtSoapQName(QLatin1String("Faultcode")), codeStr));
}
...
void QtSoapMessage::setFaultString(const QString &s)
{
    ...
    QtSoapType &node = body()[QtSoapQName(QLatin1String("Fault"), QLatin1String(SOAPv11_ENVELOPE))];
    QtSoapStruct &fault = reinterpret_cast<QtSoapStruct &>(node);
    fault.insert(new QtSoapSimpleType(QtSoapQName(QLatin1String("Faultstring")), s));
}
...
void QtSoapMessage::addFaultDetail(QtSoapType *detail)
{
    ...
    QtSoapType &node = body()[QtSoapQName(QLatin1String("Fault"), QLatin1String(SOAPv11_ENVELOPE))];
    QtSoapStruct &fault = reinterpret_cast<QtSoapStruct &>(node);
    if (!fault[QtSoapQName(QLatin1String("Faultdetail"), QLatin1String(SOAPv11_ENVELOPE))].isValid())
        fault.insert(new QtSoapStruct(QtSoapQName(QLatin1String("Faultdetail"), QLatin1String(SOAPv11_ENVELOPE))));

    QtSoapType &node2 = fault[QtSoapQName(QLatin1String("Faultdetail"), QLatin1String(SOAPv11_ENVELOPE))];
...
QtSoapTypeFactory::QtSoapTypeFactory()
{
    ...
    registerHandler(QLatin1String("struct"), structConstructor);
    registerHandler(QLatin1String("array"), arrayConstructor);
    registerHandler(QLatin1String("string"), basicTypeConstructor);
    registerHandler(QLatin1String("normalizedstring"), basicTypeConstructor);
    registerHandler(QLatin1String("token"), basicTypeConstructor);
    registerHandler(QLatin1String("language"), basicTypeConstructor);
    registerHandler(QLatin1String("name"), basicTypeConstructor);
    registerHandler(QLatin1String("ncname"), basicTypeConstructor);
    registerHandler(QLatin1String("id"), basicTypeConstructor);
    registerHandler(QLatin1String("idref"), basicTypeConstructor);
    registerHandler(QLatin1String("entity"), basicTypeConstructor);
    registerHandler(QLatin1String("nmtoken"), basicTypeConstructor);
    registerHandler(QLatin1String("nmtokens"), basicTypeConstructor);
    registerHandler(QLatin1String("boolean"), basicTypeConstructor);
    registerHandler(QLatin1String("decimal"), basicTypeConstructor);
    registerHandler(QLatin1String("integer"), basicTypeConstructor);
    registerHandler(QLatin1String("nonpositiveinteger"), basicTypeConstructor);
    registerHandler(QLatin1String("negativeinteger"), basicTypeConstructor);
    registerHandler(QLatin1String("int"), basicTypeConstructor);
    registerHandler(QLatin1String("long"), basicTypeConstructor);
    registerHandler(QLatin1String("short"), basicTypeConstructor);
    registerHandler(QLatin1String("byte"), basicTypeConstructor);
    registerHandler(QLatin1String("nonnegativeinteger"), basicTypeConstructor);
    registerHandler(QLatin1String("unsignedlong"), basicTypeConstructor);
    registerHandler(QLatin1String("unsignedint"), basicTypeConstructor);
    registerHandler(QLatin1String("unsignedshort"), basicTypeConstructor);
    registerHandler(QLatin1String("unsignedbyte"), basicTypeConstructor);
    registerHandler(QLatin1String("positiveinteger"), basicTypeConstructor);
    registerHandler(QLatin1String("float"), basicTypeConstructor);
    registerHandler(QLatin1String("double"), basicTypeConstructor);
    //registerHandler(QLatin1String("hexBinary", basicTypeConstructor);
    //registerHandler(QLatin1String("base64Binary", basicTypeConstructor);
    registerHandler(QLatin1String("other"), structConstructor);
}
...
QScopedPointer<QtSoapTypeFactory> QtSoapTypeFactory::s_instance;
...
QtSoapTypeFactory &QtSoapTypeFactory::instance()
{
    ...
    s_instance.reset(new QtSoapTypeFactory());
    return *s_instance;
}
...
bool QtSoapTypeFactory::registerHandler(const QString &name, QtSoapTypeConstructorBase *handler)
{
    if (typeHandlers.find(name) != typeHandlers.end()) {
        errorStr = QLatin1String("A handler for ") + name + QLatin1String(" is already registered.");
        return false;
    }
...
QtSmartPtr<QtSoapType> QtSoapTypeFactory::soapType(QDomNode node) const
{
    ...
    QDomAttr attr = elem.attributeNode(QLatin1String("type"));
    ...
    if (attr.isNull() || !constructor) {
        ...
        if (node.firstChild().isElement()) {
            if (localName(node.nodeName().toLower()) == QLatin1String("array")) {
                it = typeHandlers.find(QLatin1String("array"));
            } else
                it = typeHandlers.find(QLatin1String("struct"));
        } else
            it = typeHandlers.find(QLatin1String("string"));
        if (it != typeHandlers.end())
            constructor = *it;
    }
...
void QtSoapHttpTransport::readResponse(QNetworkReply *reply)
{
    ...
    default:
        {
            soapResponse.setFaultCode(QtSoapMessage::Client);
            soapResponse.setFaultString(QString(QLatin1String("Network transport error (%1): %2")).arg(reply->error()).arg(reply->errorString()));
        }
        break;
    }
...
QScopedPointer<QtSoapNamespaces> QtSoapNamespaces::s_instance;
...
QtSoapNamespaces &QtSoapNamespaces::instance()
{
    ...
    s_instance.reset(new QtSoapNamespaces());
    return *s_instance;
}
...
QtSoapNamespaces::QtSoapNamespaces() :
    namespacesMutex(QMutex::Recursive)
{
}
...
{
    QMutexLocker lock(&namespacesMutex);
