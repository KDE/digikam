    ...
    m_type(Undefined), m_resourceElements()
{
    ...
    QStringList tmp = resourceTypeAsStr.simplified().split(QLatin1String(":"));
    if (tmp.size() != 5)
    {
        return;
    }

    if (tmp[0] != QLatin1String("urn"))
    {
        return;
    }
    ...
    if (tmp[1].compare(QLatin1String("schemas-upnp-org")) != 0)
    {
        flags = 0x01;
    ...
    if (tmp[2].compare(QLatin1String("device")) == 0)
    {
        flags |= 0x04;
    }
    else if (tmp[2].compare(QLatin1String("service")) == 0)
    {
        flags |= 0x08;
    }
    else
...
QString HResourceType::toString(Tokens tokens) const
{
    ...
    if (tokens & Type)
    {
        if (appendDelim) { retVal.append(QLatin1Char(':')); }
