...
private:
    ...
    static bool checkValues(const HValueRange& val, QString* err = 0)
    {
        ...
        if (min > max)
        {
            if (err)
            {
                *err = QLatin1String("Minimum value cannot be larger than the maximum");
            }
            return false;
        }
        ...
        if (max - min < step)
        {
            if (err)
            {
                *err = QLatin1String("Step value cannot be larger than the entire range");
            }
            return false;
        }
...
public:
        ...
        HValueRange* retVal, QString* err = 0)
    {
        ...
        {
            if (err) { *err = QLatin1String("Invalid maximum value"); }
            return false;
        }
        ...
        {
            if (err) { *err = QLatin1String("Invalid minimum value"); }
            return false;
        }
        ...
        {
            if (err) { *err = QLatin1String("Invalid step value"); }
            return false;
        }
        ...
        switch(dataType)
        {
            ...
            default:
                Q_ASSERT(false);
                if (err) { *err = QLatin1String("Invalid data type"); }
                return false;
        }
