...
public:
    ...
    bool parse(const QString& arg, HValidityCheckLevel checkLevel)
    {
        ...
        HUdn udn;
        qint32 indx = tmp.indexOf(QLatin1String("::"));
        ...
        QStringList parsed = tmp.split(QLatin1Char(':'));
        if (parsed.size() < 2)
        {
            HLOG_WARN(QString(QLatin1String("Invalid resource identifier: %1")).arg(arg));
            return false;
        }

        if (!udn.isValid(checkLevel))
        {
            if (parsed[0] == QLatin1String("ssdp") && parsed[1] == QLatin1String("all"))
            {
                m_type = HDiscoveryType::All;
                m_contents = QLatin1String("ssdp:all");
                return true;
            }
        }

        if (parsed[0] == QLatin1String("upnp") && parsed[1] == QLatin1String("rootdevice"))
        {
            m_udn = udn;
            ...
            if (m_udn.isValid(checkLevel))
            {
                ...
                m_contents = QString(QLatin1String("%1::upnp:rootdevice")).arg(udn.toString());
            }
            else
            {
                m_type = HDiscoveryType::RootDevices;
                m_contents = QLatin1String("upnp:rootdevice");
            }
            return true;
        }
        else if (parsed[0] == QLatin1String("uuid"))
        {
            udn = HUdn(parsed[1]);
            if (udn.isValid(checkLevel))
            {
        ...
        if (parse(resourceType))
        {
            ...
            if (m_udn.isValid(checkLevel))
            {
                ...
                m_contents = QString(QLatin1String("%1::%2")).arg(
            ...
            else
            {
                ...
                m_contents = QString(QLatin1String("%1")).arg(resourceType.toString());
            }
        ...
        const HUdn& udn, const HResourceType& rt, HValidityCheckLevel checkLevel)
    {
        if (udn.isValid(checkLevel))
        {
            ...
            m_contents = QString(QLatin1String("%1::%2")).arg(udn.toString(), rt.toString());
        }
        else
        {
            switch(rt.type())
            ...
            m_contents = QString(QLatin1String("%1")).arg(rt.toString());
        }
        ...
        h_ptr(new HDiscoveryTypePrivate())
{
    if (udn.isValid(checkLevel))
    {
        if (isRootDevice)
        {
            ...
            h_ptr->m_contents =
                QString(QLatin1String("%1::upnp:rootdevice")).arg(udn.toString());
        }
        else
        {
        ...
        h_ptr(new HDiscoveryTypePrivate())
{
    if (h_ptr->parse(resourceType) && udn.isValid(checkLevel))
    {
        h_ptr->m_udn = udn;
        h_ptr->m_contents =
            QString(QLatin1String("%1::%2")).arg(udn.toString(), resourceType.toString());
...
{
    static HDiscoveryType retVal(QLatin1String("upnp:rootdevice"));
    return retVal;
}

HDiscoveryType HDiscoveryType::createDiscoveryTypeForAllResources()
{
    static HDiscoveryType retVal(QLatin1String("ssdp:all"));
    return retVal;
}
