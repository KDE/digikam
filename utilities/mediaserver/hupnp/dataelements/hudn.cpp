...
HUdn::HUdn(const QUuid& value) :
    m_value(value.toString().remove(QLatin1Char('{')).remove(QLatin1Char('}')))
{
}
...
{
    if (m_value.startsWith(QLatin1String("uuid:")))
    {
...
QString HUdn::toString() const
{
    ...
    else if (m_value.startsWith(QLatin1String("uuid:")))
    {
        return m_value;
    }

    return QString(QLatin1String("uuid:")).append(m_value);
}
