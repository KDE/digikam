...
bool HProductToken::isValid(HValidityCheckLevel checkLevel) const
{
    ...
    bool ok = false;
    qint32 separatorIndex = m_productVersion.indexOf(QLatin1Char('.'));
    if (separatorIndex < 0)
    {
    ...
    if (ok)
    {
        m_productVersion.mid(
            separatorIndex+1, m_productVersion.indexOf(QLatin1Char('.'), separatorIndex+1)).toInt(&ok);
    }
...
QString HProductToken::toString() const
{
    ...
    return QString(QLatin1String("%1/%2")).arg(m_token, m_productVersion);
}
...
qint32 HProductToken::minorVersion()
{
    ...
    qint32 separatorIndex = tokenVersion.indexOf(QLatin1Char('.'));
    if (separatorIndex < 0)
    {
        return -1;
    }
    ...
    qint32 minTmp = tokenVersion.mid(
        separatorIndex+1, tokenVersion.indexOf(QLatin1Char('.'), separatorIndex+1)).toInt(&ok);
...
private:
    ...
    bool parse(const QString& tokens)
    {
        ...
        qint32 i = tokens.indexOf(QLatin1Char('/')), j = 0, lastDelim = 0;
        if (i < 0)
        {
            return false;
        }
...
public:
        ...
        m_originalTokenString(tokens.simplified()), m_productTokens()
    {
        ...
        if (m_originalTokenString.contains(QLatin1Char(',')))
        {
            ...
            ok = parse(QString(m_originalTokenString).replace(QLatin1Char(','), QLatin1Char(' ')));
            if (ok)
            {
        ...
        if (!ok)
        {
            ...
            QRegExp rexp(QLatin1String("(\\b|\\s+)UPnP/"));
