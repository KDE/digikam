        ...
        h_ptr(new HServiceInfoPrivate())
{
    ...
    {
        errTmp = QLatin1String("Invalid service ID");
    }
    else if (!serviceType.isValid())
    {
        errTmp = QLatin1String("Invalid service type");
    }
    else if (controlUrl.isEmpty() || !controlUrl.isValid())
    {
        errTmp = QLatin1String("Invalid control URL");
    }
    else if (eventSubUrl.isEmpty() || !eventSubUrl.isValid())
    {
        errTmp = QLatin1String("Invalid event sub URL");
    }
    else if (scpdUrl.isEmpty() || !scpdUrl.isValid())
    {
        errTmp = QLatin1String("Invalid SCPD URL");
    }
    else
    {
