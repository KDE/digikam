...
public:
        ...
        m_suffix(), m_elements()
    {
        ...
        QStringList tmp = arg.simplified().split(QLatin1String(":"));
        if (tmp.size() < 4)
        {
            HLOG_WARN_NONSTD(
                QString(QLatin1String("Invalid service identifier [%1]")).arg(arg));

            return;
        }

        if (tmp[0].compare(QLatin1String("urn"), Qt::CaseInsensitive) != 0)
        {
            HLOG_WARN_NONSTD(
                QString(QLatin1String("Invalid service identifier [%1]")).arg(arg));
        ...
        if (tmp[1].compare(QLatin1String("upnp-org"), Qt::CaseInsensitive) != 0)
        {
            tmp[1] = tmp[1].replace(QLatin1Char('.'), QLatin1Char('-'));
            if (tmp[1].isEmpty())
            {
        ...
        bool warned = false;
        if (tmp[2].compare(QLatin1String("serviceId"), Qt::CaseInsensitive) != 0)
        {
            HLOG_WARN_NONSTD(QString(QLatin1String("Invalid service identifier [%1]")).arg(arg));
            warned = true;
        ...
        if (tmp[3].isEmpty())
        {
            if (!warned)
            {
                HLOG_WARN(QString(QLatin1String("Invalid service identifier [%1]")).arg(arg));
            }
            return;
        }
        ...
        {
            m_suffix.append(QLatin1Char(':')).append(tmp[i]);
        }
...
bool HServiceId::isValid(HValidityCheckLevel checkLevel) const
{
    ...
    return h_ptr->m_elements.size() >= 4 &&
           h_ptr->m_elements[0] == QLatin1String("urn") &&
           h_ptr->m_elements[2] == QLatin1String("serviceId");
}
...
bool HServiceId::isStandardType() const
{
    ...
    return h_ptr->m_elements[1] == QLatin1String("upnp-org");
}
...
QString HServiceId::urn(bool completeUrn) const
{
    ...
    if (completeUrn)
    {
        retVal.append(QLatin1String("urn:"));
    }
...
{
    return h_ptr->m_elements.join(QLatin1String(":"));
}
