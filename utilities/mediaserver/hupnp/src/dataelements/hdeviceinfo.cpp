...
bool HDeviceInfoPrivate::setModelDescription(const QString& modelDescription)
{
    ...
    {
        HLOG_WARN(QString(
            QLatin1String("modelDescription [%1] longer than 64 characters")).arg(modelDescription));
    }
...
bool HDeviceInfoPrivate::setModelName(const QString& modelName)
{
    ...
    {
        HLOG_WARN(QString(
            QLatin1String("modelName [%1] longer than 32 characters: [%1]")).arg(modelName));
    }
...
bool HDeviceInfoPrivate::setModelNumber(const QString& modelNumber)
{
    ...
    {
        HLOG_WARN(QString(
            QLatin1String("modelNumber [%1] longer than 32 characters: [%1]")).arg(modelNumber));
    }
...
bool HDeviceInfoPrivate::setSerialNumber(const QString& serialNumber)
{
    ...
    {
        HLOG_WARN(QString(
            QLatin1String("serialNumber [%1] longer than 64 characters: [%1]")).arg(serialNumber));
    }
...
bool HDeviceInfoPrivate::setUpc(const QString& upc)
{
    ...
    if (upc.size() > 13 || upc.size() < 12)
    {
        ...
        HLOG_WARN_NONSTD(QString(
            QLatin1String("UPC should be 12-digit, all-numeric code. Encountered: [%1].")).arg(
                upc));
    }
    else
    {
        for(qint32 i = 0; i < upc.size(); ++i)
        {
            ...
            if ((i == 6 && !ch.isSpace() && ch != QLatin1Char('-') && upc.size() == 13) ||
                !ch.isDigit())
            {
        ...
        h_ptr(new HDeviceInfoPrivate())
{
    ...
    {
        errTmp = QString(QLatin1String("Invalid device type: [%1]")).arg(deviceType.toString());
    }
    else if (!tmp->setFriendlyName(friendlyName))
    {
        errTmp = QString(QLatin1String("Invalid friendly name: [%1]")).arg(friendlyName);
    }
    else if (!tmp->setManufacturer(manufacturer))
    {
        errTmp = QString(QLatin1String("Invalid manufacturer: [%1]")).arg(manufacturer);
    }
    else if (!tmp->setModelName(modelName))
    {
        errTmp = QString(QLatin1String("Invalid model name: [%1]")).arg(modelName);
    }
    else if (!tmp->setUdn(udn, checkLevel))
    {
        errTmp = QString(QLatin1String("Invalid UDN: [%1]")).arg(udn.toString());
    }
        ...
        h_ptr(new HDeviceInfoPrivate())
{
    ...
    h_ptr->setManufacturerUrl (QUrl(manufacturerUrl.toString()));
    h_ptr->setModelDescription(modelDescription);

    h_ptr->setModelNumber     (modelNumber);
    h_ptr->setModelUrl        (QUrl(modelUrl.toString()));
    h_ptr->setSerialNumber    (serialNumber);
    h_ptr->setUpc             (upc);
    h_ptr->setIcons           (icons);
    h_ptr->setPresentationUrl (QUrl(presentationUrl.toString()));
}
