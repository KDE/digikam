...
void HotPixelsTool::prepareFinal()
{
    ...
    setFilter(dynamic_cast<DImgThreadedFilter*>(new HotPixelFixer(iface.getOriginal(), this, d->hotPixelsList, interpolationMethod)));
}
...
{
    ImageIface iface(0, 0);
    iface.putOriginal(i18n("Hot Pixels Correction"), filter()->filterAction(), filter()->getTargetImage().bits());
}
