...
void ModelTest::rowCount()
{
    ...
    if (rows > 0)
    {
        QVERIFY(model->hasChildren(topIndex) == true);
    }
    ...
    if (secondLevelIndex.isValid())   // not the top level
    {
        ...
        if (rows > 0)
        {
            QVERIFY(model->hasChildren(secondLevelIndex) == true);
        }
    }
...
void ModelTest::hasIndex()
{
    ...
    QVERIFY(model->hasIndex(-2, -2) == false);
    QVERIFY(model->hasIndex(-2, 0) == false);
    QVERIFY(model->hasIndex(0, -2) == false);

    int rows = model->rowCount();
    int columns = model->columnCount();

    // check out of bounds
    QVERIFY(model->hasIndex(rows, columns) == false);
    QVERIFY(model->hasIndex(rows + 1, columns + 1) == false);

    if (rows > 0)
    {
        QVERIFY(model->hasIndex(0, 0) == true);
    }
...
void ModelTest::index()
{
    ...
    QVERIFY(model->index(0, 0).isValid() == true);
...
void ModelTest::checkChildren(const QModelIndex& parent, int currentDepth)
{
    ...
    if (rows > 0)
    {
        QVERIFY(model->hasChildren(parent) == true);
    }

    //qDebug() << "parent:" << model->data(parent).toString() << "rows:" << rows
    //         << "columns:" << columns << "parent column:" << parent.column();

    QVERIFY(model->hasIndex(rows + 1, 0, parent) == false);
    ...
    for (int r = 0; r < rows; ++r)
    {
        ...
        QVERIFY(model->hasIndex(r, columns + 1, parent) == false);

        for (int c = 0; c < columns; ++c)
        {
            QVERIFY(model->hasIndex(r, c, parent) == true);
            QModelIndex index = model->index(r, c, parent);
            // rowCount() and columnCount() said that it existed...
            QVERIFY(index.isValid() == true);
            ...
            QVERIFY(index.row() == r);
            QVERIFY(index.column() == c);
            // While you can technically return a QVariant usually this is a sign
            // of an bug in data()  Disable if this really is ok in your model.
            QVERIFY(model->data(index, Qt::DisplayRole).isValid() == true);
            ...
            QVERIFY(model->parent(index) == parent);
...
void ModelTest::data()
{
    ...
    QVERIFY(model->setData(QModelIndex(), QLatin1String("foo"), Qt::DisplayRole) == false);
    ...
    if (textAlignmentVariant.isValid())
    {
        ...
        QVERIFY(alignment == (alignment & (Qt::AlignHorizontal_Mask | Qt::AlignVertical_Mask)));
    }
