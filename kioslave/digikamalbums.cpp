...
void kio_digikamalbums::special(const QByteArray& data)
{
    ...
    kDebug(kioslavesAreaCode) << "kio_digikamalbums::special " << kurl;
...
{
    kDebug(kioslavesAreaCode) << " : " << url.url();
...
{
    kDebug(kioslavesAreaCode) << "Src: " << src.path() << ", Dst: " << dst.path();
...
{
    kDebug(kioslavesAreaCode) << "Src: " << src << ", Dst: " << dst;
...
{
    kDebug(kioslavesAreaCode) << " : " << url.path();
...
extern "C"
{
    KDE_EXPORT int kdemain(int argc, char **argv)
    {
        ...
        if (argc != 4) {
            kDebug(kioslavesAreaCode) << "Usage: kio_digikamalbums  protocol domain-socket1 domain-socket2";
            exit(-1);
        }
